 ...
 * @author paulmartin / http://blog.mobiushorizons.com/
 */

THREE.FirstPersonFlightControls = function ( object, domElement ) {
  ...
  /* aerodynamics */
	...
	this.update = function( deltaT ) {
		...
		if ( this.heightSpeed ) {
			...
			this.autoSpeedFactor = deltaT * ( heightDelta * this.heightCoef );
		...
		var acceleration = new THREE.Vector3(this.moveUp-this.moveDown, this.moveLeft - this.moveRight,this.moveForward - this.moveBackward).multiplyScalar(3);
		var dragAccel = new THREE.Vector3(0,0,0);
		dragAccel.copy(this.velocity).setLength(this.velocity.lengthSq() * this.drag);
		var deltaV = new THREE.Vector3(0,0,0)
			.subVectors(acceleration,dragAccel); // (A-D)dT
		...
		var actualLookSpeed = deltaT * this.lookSpeed;
